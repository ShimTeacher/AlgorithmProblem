...
public class Main {
	...
	private void run() throws Exception{
		...
		System.out.println(getMaxCount(0,0,BOARD));
...
//	BBCDE
	...
	static int getMaxCount(int startX, int startY, char BOARD[][])
	{
		int ret = 0;
		...
		for (int direction = 0; direction < 2; direction++) {
			...
			changeValue(startX,startY,nextX,nextY, BOARD);
			
			int ret1 = CountCandy(BOARD);
			
			changeValue(nextX,nextY,startX,startY, BOARD);
			int ret2= getMaxCount(nextX, nextY, BOARD);
	...
	static void changeValue(int oldX, int oldY, int newX, int newY,char BOARD[][])
	{
